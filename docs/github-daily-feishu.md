...
这项独立任务每天北京时间 09:17 汇总 GitHub 近 7 天的新锐仓库，并通过仓库现有的飞书通知 Secret 发送一张消息卡片。鉴于 GitHub cron 在本仓库历史上约有 30 分钟排队延迟，工作流会在 08:30、08:45 和 09:00 提前申请运行；最先启动的任务等待到 09:17 再发送，后续任务通过上海自然日内的成功记录自动跳过，因此每天最多发送一次。它不会改变 TrendRadar 原有的新闻抓取任务。
...
- 运行时间：编辑 `.github/workflows/github-daily-hot-v2.yml` 的 cron。工作流使用 `Asia/Shanghai` 时区，在 08:30、08:45 和 09:00 提前排队，并在 09:17 执行发送。
